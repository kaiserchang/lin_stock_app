...
class LinJiaYangEngine:
    ...
    def is_attack_k(self, idx):
        """
        判斷是否為攻擊K線
        條件：長紅K (漲幅 > 3%) 且帶量 (大於 5 日均量 1.5 倍)
        進階條件：收盤價必須突破過去 20 天的波段高點 (Swing High)
        """
        # 因為需要回推 20 天找高點，所以防呆機制改為 idx < 20
        ...
        # --- 新增條件 4：突破波段高點 ---
        # 尋找 idx 往前推 20 天內的最高價 (不包含今天)
        swing_high_20 = self.df['High'].iloc[idx-20:idx].max()
        cond4 = curr['Close'] > swing_high_20 # 今天的收盤價必須站上這座山頭
